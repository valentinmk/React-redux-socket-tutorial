...
В роли учебной задачи представлена страничка чата с каким-то "ботом"", который работает на стороне сервера, через WebSocket. Бот при этом выполняет эхо ваших запросов (мы тут не рассматриваем серверную часть вообще).
