...
В нашем обработчике получаем событие от сокета, извлекаем из него сообщение и передаем в редукс через экшен.
...
Подробнее. `action.message_send` - это о чем? Все, что мы кладем в редукс появляется в процессе обработки `store => next => action =>` в этих переменных. Когда мы запускаем экшен, то в этой переменной передается все с  чем мы этот экшен запустили.
